# Thanks @AlexisAhmed for his awesome docker secure script! 
...
openssl req -new -x509 -days 365 -key ca-key.pem -passin pass:$PASSWORD -sha256 -out ca.pem -subj "/C=TR/ST=./L=./O=./CN=$SERVER" -addext "subjectAltName = DNS:$SERVER"
...
sh -c 'echo "extendedKeyUsage = clientAuth\nsubjectAltName=DNS:$SERVER" > extfile.cnf'
